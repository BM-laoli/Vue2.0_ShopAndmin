<template>
  <el-table :data="list" style="width: 100%" :border="false">
    ...
    <el-table-column prop="role" label="职位" width="300"> </el-table-column>
    <el-table-column prop="status" label="状态" width="200">
      {{ list.status == true ? "启用" : "停用" }}
    </el-table-column>
    <el-table-column label="操作" #default="{row}">
      <el-button type="primary" size="mini" @click="$emit('show-dialog', row)"
        >编辑</el-button
      >
      <el-button
        type="warning"
      ...
      <el-button
        ...
        plain
        size="mini"
        @click="$emit('stop-use', row)"
        >{{ row.status == true ? "停用" : "启用" }}</el-button
      >
      <el-button
        type="danger"
        size="mini"
        @click="$emit('delete-user', row._id)"
        >删除</el-button
      >
    </el-table-column>
  </el-table>
