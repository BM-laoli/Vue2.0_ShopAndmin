...
export default {
  ...
  data() {
    return {
      ...
      userFormRules: {
        ...
        rule: [{ required: true, message: "请勾选权限", trigger: "blur" }]
      },
      // 编辑表单验证规则
      editUserFormRules: {
        username: [
  ...
  methods: {
    ...
    async stopUse(row) {
      ...
      const data = await this.$http.put(`rest/operation_user/${row._id}`, row);
      // console.log(data);
      if (data.status !== 200) {
    ...
    async add() {
      ...
      const res = await this.$http.post("rest/operation_user", this.userForm);
      console.log(res);
      if (res.status !== 200) {
