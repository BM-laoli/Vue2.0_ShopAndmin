...
import { updateRight } from '../../api/shops/index'
export default {
  name: 'ShopRight',
  components: {
    breadCrumbs,
  ...
  methods: {
    async confirmRight () {
      try {
        if (this.right1.all_public_num && this.right1.price !== '') {
          this.right1.level = 1
          const { data: res } = await updateRight(this.right1)
          this.$message.success('保存成功')
        } else {
          this.$message.error('保存失败,请填写正确的参数')
        }
        if (this.right2.all_public_num && this.right2.price !== '') {
          this.right2.level = 2
          const { data: res } = await updateRight(this.right2)
          this.$message.success('保存成功')
        } else {
          this.$message.error('保存失败,请填写正确的参数')
        }
        if (this.right3.all_public_num && this.right3.price !== '') {
          this.right3.level = 3
          const { data: res } = await updateRight(this.right3)
          this.$message.success('保存成功')
        } else {
          this.$message.error('保存失败,请填写正确的参数')
        }
